...
import { RankIcon } from '@shared/assets/icons';
import type { TableRow } from '../../model/types';
import PlacesColumnCell from './PlacesColumnCell';
...
const columns = [
  columnHelper.accessor((row) => row.common.name, {
        ...
        <Flex gap="sm" align="center">
          ...
          <Flex direction="column" justify="center">
            ...
            {data.common.season !== undefined && (
              <Text size="xs" fw={700}>
                <FormattedMessage defaultMessage="Сезон " /> {data.common.season}
              </Text>
            )}
          </Flex>
        </Flex>
    ...
    header: () => <FormattedMessage defaultMessage="Герой" />,
  }),
  columnHelper.accessor((row) => row.common.rank, {
    id: 'rank',
    header: () => <FormattedMessage defaultMessage="Ранг" />,
    size: 70,
  ...
  columnHelper.accessor((row) => row.skills.skill1, {
    ...
    header: () => (
      <Stack align="center" gap={2}>
        <FormattedMessage defaultMessage="Навык" />
  ...
  columnHelper.accessor((row) => row.upgradeParams, {
    id: 'upgradeParams',
    header: () => <FormattedMessage defaultMessage="Моя прокачка героя" />,
    cell: (props) => {
      const withUpgrades = getWithUpgrades(props)
      const withAfterUpgrades = getWithAfterUpgrades(props)
        ...
        <Flex justify="center">
          <Stack w={100} gap={4}>
            ...
            <Stack gap={0}>
              {data.cardsForBar > 0 && (
                <Text size="sm">
                  <FormattedMessage defaultMessage="До полосы: " /> {data.cardsForBar}
                </Text>
              )}
              {data.cardsForStar > 0 && (
                <Text size="sm">
                  <FormattedMessage defaultMessage="До звезды: " /> {data.cardsForStar}
                </Text>
              )}
              {withAfterUpgrades &&
                rowData.upgradeParams.before.upgradeStars < HeroHelper.getMaxStars(rowData.common.rank) && (
                  <Text size="sm">
                    <FormattedMessage defaultMessage="Осталось: " /> {data.leftCards}
                  </Text>
                )}
            </Stack>
          </Stack>
  ...
  columnHelper.accessor((row) => row.params, {
    id: 'params',
    header: () => <FormattedMessage defaultMessage="Мои карты" />,
    minSize: 175,
    cell: (props) => (
      <Flex direction="column" align="center">
        <Text size="sm">
          <FormattedMessage defaultMessage="Карт в наличии: " /> {props.row.original.upgradeParams.before.leftCards}
        </Text>
        <Text size="sm">
          <FormattedMessage defaultMessage="Карт самовыбора: " /> {props.row.original.params.distributionCards}
        </Text>
        <Text size="sm">
          <FormattedMessage defaultMessage="Всего карт: " />{' '}
          {props.row.original.upgradeParams.before.leftCards + props.row.original.params.distributionCards}
        </Text>
      </Flex>
    ),
  }),
  columnHelper.accessor((row) => row.common.placesIds, {
    id: 'places',
    header: () => <FormattedMessage defaultMessage="Где можно получить" />,
    minSize: 200,
    cell: (props) => (
      <Flex direction="column" align="flex-start">
        {props.getValue().map((placeId) => (
          <PlacesColumnCell key={placeId} placeId={placeId} season={props.row.original.common.season} />
        ))}
      </Flex>
    ),
