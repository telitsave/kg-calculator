...
const DetailsResultsTable: FC<Props> = memo(({ className, data }) => {
        ...
        return (
          <Table.Tr bg={isEliteRow ? '#b98abf' : isBossRow ? '#cc747c' : ''}>
            ...
            <Table.Td align="right">
              <Text>{StringHelper.getFormatNumber(row.power)}</Text>
            </Table.Td>
            <Table.Td align="right">
              <Text>{StringHelper.getFormatNumber(row.scoreByOne)}</Text>
            </Table.Td>
            <Table.Td align="right">
              <Text>{StringHelper.getFormatNumber(row.scoreTotal)}</Text>
            </Table.Td>
          </Table.Tr>
        )
      }),
  ...
  return (
    <Table className={className} border={1}>
      <Table.Thead>
        <Table.Tr>
          <Table.Th>Волна</Table.Th>
          <Table.Th align="right">Мощь</Table.Th>
          <Table.Th align="right">Очки одному участнику</Table.Th>
          <Table.Th align="right">Очки на всех участников</Table.Th>
        </Table.Tr>
      </Table.Thead>
